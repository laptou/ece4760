...
listen_state_t listen_state = ls_active;
unsigned int listen_state_last_time = 0;
unsigned int chirp_counter = 0;
...
static PT_THREAD(protothread_fft(struct pt *pt))
{
    ...
    while (1)
    {
        ...
        fillRect(250, 60, 176, 40, RED);

        unsigned int listen_state_duration = listen_state_last_time - now;
        switch (listen_state)
        {
        case ls_wait:
            if (abs(max_freqency - 2300) < 20 && listen_state_duration > 100)
            {
                listen_state = ls_active;
                listen_state_last_time = now;
            }
            break;
        case ls_active:
            setCursor(250, 80);
            writeString("chirp detected");
            if (abs(max_freqency - 2300) > 20 && listen_state_duration > 100)
            {
        ...
        sprintf(chirptext, "%d %d %d %d", chirp_counter, listen_state, listen_state_last_time, now);
        setCursor(250, 60);
