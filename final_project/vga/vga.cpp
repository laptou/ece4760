...
void vga_init()
{
  // Choose which PIO instance to use (there are two instances, each with 4
  // state machines)
  PIO pio = pio0;
  ...
  // than 32 instructions, then an error message will get thrown at these lines
  // of code.
  //
  ...
  // Why not create these programs here? By putting the initialization function
  // in the pio file, then all information about how to use/setup that state
  // machine is consolidated in one place. Here in the C, we then just import
  // and use it.
  ...
  pio_enable_sm_mask_in_sync(
      pio, ((1u << hsync_sm) | (1u << vsync_sm) | (1u << rgb_sm))
  );
...
void vga_pixel(short x, short y, char color)
{
  ...
  if (pixel & 1) {
    vga_data_array[pixel >> 1] =
        (vga_data_array[pixel >> 1] & TOPMASK) | (color << 3);
  } else {
    vga_data_array[pixel >> 1] =
        (vga_data_array[pixel >> 1] & BOTTOMMASK) | (color);
  }
}
...
void vga_stroke_circle_helper(
    short x0, short y0, short r, unsigned char cornername, char color
)
{
...
void vga_fill_circle_helper(
    short x0, short y0, short r, unsigned char cornername, short delta,
    char color
)
{
